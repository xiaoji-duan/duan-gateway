...
public class AccessGrantFilter extends ZuulFilter {
	...
	public boolean shouldFilter() {
		...
		return !ctx.getBoolean("authorizing") 
				...
				&& !ctx.getRequest().getRequestURI().startsWith("/aba/")
				&& !ctx.getRequest().getRequestURI().startsWith("/abb/")
        		&& !ctx.getRequest().getRequestURI().endsWith("/shortapplication.json")
        		&& !ctx.getRequest().getRequestURI().endsWith("/access_token")
        		&& !ctx.getRequest().getRequestURI().endsWith("/refresh_token")
	...
	public Object run() throws ZuulException {
        ...
        if (resp != null && !ctx.getRequest().getRequestURI().startsWith("/abd/" + subdomain + "/")) {
        	...
        	if (isLogin) {
        		...
        		System.out.println("[" + subdomain + "] [" + verifyType + "] [" + requestURI + "]");
        	}
        	
        	String scheme = ctx.getRequest().getHeader("x-forwarded-proto");
        	if (StringUtils.isEmpty(scheme)) {
        		scheme = ctx.getRequest().getScheme();
        	}

        	String redirect = scheme + "://" + host + "/abd";
        	...
        	if (isLogin && !"MWXING".equals(verifyType)) {
        		  // 过滤该请求
        	...
        	} else if (isLogin && "MWXING".equals(verifyType)) {
        		ctx.setSendZuulResponse(false);
        		ctx.setResponseStatusCode(401);
        		return null;
        	}
